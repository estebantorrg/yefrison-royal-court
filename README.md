...
*   **The Oracle of Yefris (`/oracle`, also embedded on `/cult`):** A fully interactive AI chat interface powered by Google's Gemma 4 31B model via the `@google/genai` SDK, governed by custom system prompts enforcing the persona of an oblivion-obsessed cult leader. A secondary, cheaper model (Gemma 4 26B) compacts long chat histories server-side to stay within context limits.
...
*   **Ultra-Responsive Typewriter Stream:** The frontend UI artificially streams the edge-buffered response via a batched async generator `askYefrisStream` (~40 ticks at 4ms each). This provides a premium, hyper-fluid visual illusion that streams faster and more smoothly than native SSE latency drops.
*   **Persistent Divinations:** Multi-thread session history built entirely with client-side local storage. Threads can be dynamically created, renamed, and deleted. Answers can be exported as shareable image cards.
*   **Cult Examinations (`/games`):** Five minigames — Laser Defense, The Stare of El Homun, Woofs Per Minute, The Burden of Choice, and Flight of El Homun — with global leaderboards backed by Cloudflare KV and a server-side anti-cheat session token system (`/api/game-session`).
*   **Initiation Certificate:** Generate, download, or copy a personalized cult certificate rendered client-side with `html-to-image`.
...
*   **AI Integration:** `@google/genai` SDK (Gemma 4 31B main conversation, Gemma 4 26B history compaction) + Search Grounding
*   **Edge Backend:** Cloudflare Pages Functions (Serverless) + Cloudflare KV (leaderboards, anti-cheat sessions)
...
|---|---|
| *"yefris went to take a break. come back later."* | The free-tier Gemini API daily limit has been reached, or the per-IP rate limit (60 req/min) was exceeded. Try again later. |
| *"yefris cannot authenticate. the API key may be invalid or missing."* | The `GEMINI_API_KEY` is either missing from the Cloudflare environment variables or has been revoked. |
