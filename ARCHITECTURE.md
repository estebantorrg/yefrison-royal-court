...
The client side is a Single Page Application (SPA) built using **React 18**, **TypeScript**, and **Vite**.

- **Routing Model (`react-router-dom`):** The app uses client-side routing. Cloudflare Pages is configured via a `_redirects` file (`/* /index.html 200`) so all URI requests execute `index.tsx` first, allowing React Router to seamlessly handle navigation between `/`, `/oracle`, and `/games/*`.
...
- **Secure Edge Proxy (`/api/ask`):** The frontend never directly talks to Google's API, ensuring `VITE_GEMINI_API_KEY` is completely hidden within Cloudflare Server Secrets.
- **Streaming & Parsing (`geminiService.ts`):** 
  - To minimize perceived latency (TTFB), the Cloudflare function triggers a streaming connection with Google and immediately un-buffers tokens back to the user.
  - An inline parser strips out internal chain-of-thought metadata (`<think>` tags) and explicitly exposes search grounding data so the frontend can natively render clickable "Homun Sources".
